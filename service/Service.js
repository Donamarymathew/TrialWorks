const empdb=require('../model/Model');
let employeeSerive={}
...
employeeSerive.getAllEmployees=()=>{
    return empdb.getAllEmployees().then( async data=>{
...
employeeSerive.getEmployeeById = ((employeeId) => {
    ...
    return empdb.getEmployeeById(employeeId).then((emp) => {
        if (emp == null) {
...
employeeSerive.registerEmployee=((empObj)=>{
    ...
    validator.validateempId(empObj.id);
